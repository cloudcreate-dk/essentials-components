...
import static dk.cloudcreate.essentials.components.eventsourced.eventstore.postgresql.persistence.table_per_aggregate_type.SeparateTablePerAggregateTypeConfiguration.standardConfigurationUsingJackson;
...
class MultiTenantPostgresqlEventStoreIT {
    ...
    void setup() {
        ...
        var persistenceStrategy = new SeparateTablePerAggregateTypePersistenceStrategy(jdbi,
                                                                                       unitOfWorkFactory,
                                                                                       eventMapper,
                                                                                       List.of(standardConfigurationUsingJackson(aggregateType,
                                                                                                                                            createObjectMapper(),
    ...
    void test_loadEventsByGlobalOrder() {
        ...
        eventStore.addAggregateTypeConfiguration(standardConfigurationUsingJackson(PRODUCTS,
                                                                                   createObjectMapper(),
        ...
        var persistedProductEventsStream = eventStore.loadEventsByGlobalOrder(PRODUCTS, LongRange.from(GlobalEventOrder.FIRST_GLOBAL_EVENT_ORDER.longValue()), Optional.of(tenantId));
        ...
        persistedProductEventsStream = eventStore.loadEventsByGlobalOrder(PRODUCTS, LongRange.from(GlobalEventOrder.FIRST_GLOBAL_EVENT_ORDER.longValue()), Optional.of(TenantId.of("SomeTenant")));
        assertThat(persistedProductEventsStream).isNotNull();
        ...
        persistedOrdersEventsStream = eventStore.loadEventsByGlobalOrder(ORDERS, LongRange.from(GlobalEventOrder.FIRST_GLOBAL_EVENT_ORDER.longValue(), totalNumberOfOrderEvents), Optional.of(TenantId.of("SomeTenant")));
        assertThat(persistedOrdersEventsStream).isNotNull();
        persistedOrderEvents = persistedOrdersEventsStream.collect(Collectors.toList());
        assertThat(persistedOrderEvents).isEmpty();
    }
